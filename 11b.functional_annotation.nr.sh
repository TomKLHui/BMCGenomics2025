#!/bin/bash
set -e
# Functional annotation using the new annotation report with nr annotations

# Define the source path
cur_stage="11b.functional_annotation.nr"
...
for species in $all_species; do
    TRINOTATE_REPORT="${TRINOTATE_REPORT_DIR}/${species}/${species}.trinotate_annotation_report.nr.xls"
    FEATURE_MAP="${OUTDIR}/${species}.annot_feature_map.txt"
    COUNT_MATRIX="${MATRIX_MASTER_DIR}/${species}/${species}.isoform.counts.matrix"
    ANNOT_COUNT_MATRIX="${OUTDIR}/${species}.isoform.counts.wAnnot_nr.matrix"
    TMM_MATRIX="${MATRIX_MASTER_DIR}/${species}/${species}.isoform.TMM.EXPR.matrix"
    ANNOT_TMM_MATRIX="${OUTDIR}/${species}.isoform.TMM.EXPR.wAnnot_nr.matrix"
